...
public static partial class ImGuiWidgets
{
	...
	internal static class GridImpl
	{
		...
		{
			var widestElementHeight = itemSizes.Max(itemSize => itemSize.X);
			...
			{
				var newColumnCount = currentGridLayout.ColumnCount + 1;
				var newRowCount = (int)MathF.Ceiling(itemSizes.Length / (float)newColumnCount);
				...
				for (var i = 0; i < itemSizes.Length; i++)
				{
					...
					var maxColumnWidth = currentGridLayout.ColumnWidths[cellLocation.X];
					maxColumnWidth = Math.Max(maxColumnWidth, itemSize.X);
					currentGridLayout.ColumnWidths[cellLocation.X] = maxColumnWidth;

					var maxRowHeight = currentGridLayout.RowHeights[cellLocation.Y];
		...
		internal static void ShowRowMajor<T>(string id, IEnumerable<T> items, MeasureGridCell<T> measureDelegate, DrawGridCell<T> drawDelegate, GridOptions gridOptions)
		{
			...
			if (gridOptions.FitToContents)
			{
				var width = gridLayout.ColumnWidths.Sum();
				var height = gridLayout.RowHeights.Sum();
			...
			if (ImGui.BeginChild($"rowMajorGrid_{id}", gridOptions.GridSize, ImGuiChildFlags.None))
			{
				...
				for (var rowIndex = 0; rowIndex < gridLayout.RowCount; rowIndex++)
				{
					var isFirstRow = rowIndex == 0;
					var previousRowHeight = isFirstRow ? 0f : gridLayout.RowHeights[rowIndex - 1];

					var columnCursorX = rowTopleft.X;
					var columnCursorY = rowTopleft.Y + previousRowHeight;
					rowTopleft = new(columnCursorX, columnCursorY);
					ImGui.SetCursorScreenPos(rowTopleft);

					var cellTopLeft = ImGui.GetCursorScreenPos();
					var itemBeginIndex = rowIndex * gridLayout.ColumnCount;
					var itemEndIndex = Math.Min(itemBeginIndex + gridLayout.ColumnCount, itemListCount);
					for (var itemIndex = itemBeginIndex; itemIndex < itemEndIndex; itemIndex++)
					{
						var columnIndex = itemIndex - itemBeginIndex;
						var isFirstColumn = itemIndex == itemBeginIndex;
						var previousCellWidth = isFirstColumn ? 0f : gridLayout.ColumnWidths[columnIndex - 1];

						var cellCursorX = cellTopLeft.X + previousCellWidth;
						var cellCursorY = cellTopLeft.Y;
						cellTopLeft = new(cellCursorX, cellCursorY);
						ImGui.SetCursorScreenPos(cellTopLeft);

						var cellWidth = gridLayout.ColumnWidths[columnIndex];
						var cellHeight = gridLayout.RowHeights[rowIndex];
		...
		{
			var columnIndex = itemIndex / rowCount;
			var rowIndex = itemIndex % rowCount;
			return new(columnIndex, rowIndex);
		}

		private static GridLayout CalculateColumnMajorGridLayout(Vector2[] itemSizes, float containerHeight)
		{
			var tallestElementHeight = itemSizes.Max(itemSize => itemSize.Y);
			GridLayout currentGridLayout = new()
			{
			...
			{
				var newRowCount = currentGridLayout.RowCount + 1;
				var newColumnCount = (int)MathF.Ceiling(itemSizes.Length / (float)newRowCount);
				currentGridLayout = new()
				{
					Dimensions = new(newColumnCount, newRowCount),
					ColumnWidths = new float[newColumnCount],
		...
		internal static void ShowColumnMajor<T>(string id, IEnumerable<T> items, MeasureGridCell<T> measureDelegate, DrawGridCell<T> drawDelegate, GridOptions gridOptions)
		{
			...
			if (ImGui.BeginChild($"columnMajorGrid_{id}", gridOptions.GridSize, ImGuiChildFlags.None, ImGuiWindowFlags.HorizontalScrollbar))
			{
				...
				for (var columnIndex = 0; columnIndex < gridLayout.ColumnCount; columnIndex++)
				{
					var isFirstColumn = columnIndex == 0;
					var previousColumnWidth = isFirstColumn ? 0f : gridLayout.ColumnWidths[columnIndex - 1];

					var columnCursorX = columnTopLeft.X + previousColumnWidth;
					var columnCursorY = columnTopLeft.Y;
					columnTopLeft = new(columnCursorX, columnCursorY);
					ImGui.SetCursorScreenPos(columnTopLeft);

					var cellTopLeft = ImGui.GetCursorScreenPos();
					var itemBeginIndex = columnIndex * gridLayout.RowCount;
					var itemEndIndex = Math.Min(itemBeginIndex + gridLayout.RowCount, itemListCount);
					for (var itemIndex = itemBeginIndex; itemIndex < itemEndIndex; itemIndex++)
					{
						var rowIndex = itemIndex - itemBeginIndex;
						var isFirstRow = itemIndex == itemBeginIndex;
						var previousCellHeight = isFirstRow ? 0f : itemDimensionsWithSpacing[rowIndex - 1].Y;

						var cellCursorX = cellTopLeft.X;
						var cellCursorY = cellTopLeft.Y + previousCellHeight;
						cellTopLeft = new(cellCursorX, cellCursorY);
						ImGui.SetCursorScreenPos(cellTopLeft);
